...
def _load_results(args):
    ...
    # roots = [folder for folder in args.patch_folder.split(",")]

    for index, root in enumerate(roots):
        
        pattern = re.compile(r'output_\d+_normalized\.jsonl')
        # 遍历文件夹中的文件
        num_count = 0
        ...
        # assumes interval
        intervals = [(0, num_count - 1) for _ in range(len(roots))]
...
def majority_voting(args):
        ...
        if not patch_ids:
            ...
            if not all([x.strip() == "" for x in raw_patches]):
                ...
                maj_selected_id = max(
                    ...
                    key=lambda i: (
                        vote[raw_patches[i]],
                        -first_appear_idx[raw_patches[i]],
                    ),
                )
    ...
    if len(args.patch_folder.split(",")) == 1:
        output_folder = [Path(folder) for folder in args.patch_folder.split(",")][0]
        base_folder = os.path.join(output_folder, args.pred_dir)
    else:
        output_folder = [Path(folder) for folder in args.patch_folder.split(",")][0]
        base_folder = os.path.join(os.path.dirname(output_folder), args.pred_dir)
    
    os.makedirs(base_folder, exist_ok=True)
    pred_path = os.path.join(base_folder, 'all_preds.jsonl')
...
def normalize_patches(args):
    ...
    for output_folder in output_folders:
        pattern = re.compile(r'output_\d+_processed\.jsonl')
        # 遍历文件夹中的文件
        num_count = 0
        ...
        selected_ids = list(range(num_count))
