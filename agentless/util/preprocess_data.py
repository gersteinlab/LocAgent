        ...
        for loc in model_pred_locs.splitlines():
            ...
            if loc.startswith("class: ") and "." not in loc:
                ...
                if len(relevant_class) == 0:
                    ...
                    # 如果class有__init__函数，就返回class头和__init__函数
                    # 如果class没有函数，就返回全部
                    # 如果class有函数，但没有__init__函数，则返回第一个函数前的class内容
                    init_func = [func for func in relevant_class[0]['methods'] if func['name'] == '__init__']
                    if len(init_func) > 0:
                        line_loc.append(
                            (relevant_class[0]["start_line"], init_func[0]['end_line'])
                        )
                    elif not relevant_class[0]['methods']:
                        line_loc.append(
                            (relevant_class[0]["start_line"], relevant_class[0]["end_line"])
                        )
                    else:
                        line_loc.append(
                            (relevant_class[0]["start_line"], relevant_class[0]['methods'][0]['start_line']-1)
                        )
                ...
                # handle cases like "function: MyClass.my_method"
                if "." in loc:
            ...
            elif loc.startswith("line: ") or loc.startswith("lines: "):
                if remove_line:
                    # TODO: can recover the corresponding function instead of throwing it away
                    continue

                # loc = loc[len("line: ") :].strip()
                loc = loc.split(":")[1].strip()
                if ',' in loc:
                    lines = loc.split(',')
                    for line in lines:
                        if '-' in line or 'ｰ' in line:
                            _lines = line.split('-') if '-' in line else line.split('ｰ')
                            
                            try:
                                line_loc.append((int(_lines[0].strip()), int(_lines[1].strip())))
                            except:
                                continue
                        else:
                            try:
                                line = line.strip()
                                line_loc.append((int(line), int(line)))
                            except:
                                continue
                else:
                    # Regular expression to match different line formats
                    match = re.match(r"(\d+)\s*[-ｰ]?\s*(\d+)?", loc)
                    if match:
